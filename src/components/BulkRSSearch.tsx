...
export default function BulkRSSearch() {
  ...
  const extractRsIds = () => {
    if (!bulkText.trim()) {
      setError('Please enter text containing RS numbers');
      return;
    }

    try {
      // Use regex to extract numbers after # and before RS
      const rsIds = bulkText.match(/#(\d+)\s+RS/g) || [];
      
      if (rsIds.length === 0) {
        setError('No RS numbers found in the text');
        return;
      }
      
      // Join the IDs with commas
      const formattedIds = rsIds.map(id => {
      ...
      setError(err instanceof Error ? err.message : 'An error occurred while extracting RS numbers');
    }
  };
